...
def seed_orders_and_lines(conn):
    ...
    order_ids = []       # list of (order_id, vendor_id, po_number)
    order_line_ids = []  # list of (order_line_id, order_id, product_code, qty, price)
    ...
    for i in range(1, 201):
        ...
        conn.execute(
            """INSERT OR IGNORE INTO orders
               ...
               VALUES (?, ?, ?, ?, ?, 'USD')""",
            (po_number, vendor_id, vendor_name, order_date, status),
        )
        ...
        order_ids.append((order_id, vendor_id, po_number))
...
def seed_invoices_and_lines(conn, order_ids, order_line_ids):
    ...
    invoice_records = []  # (invoice_id, vendor_id, po_number)
    inv_line_records = []  # (inv_line_id, invoice_id, order_line_id, match_category)
    ...
    for idx, (order_id, vendor_id, po_number) in enumerate(order_ids, start=1):
        ...
        conn.execute(
            """INSERT INTO invoices
               ...
               VALUES (?, ?, ?, ?, 'USD', ?, ?, ?, 'PROCESSED')""",
            (inv_number, vendor_id, vendor_name, inv_date,
             json.dumps(raw_payload), extraction_conf, template_hash),
        )
        invoice_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        invoice_records.append((invoice_id, vendor_id, po_number))
        ...
        for ln_num, (ol_id, sku, o_qty, o_price, o_tax) in enumerate(chosen_lines, start=1):
            ...
            category = random.choices(
                ["MATCHED", "WITHIN_TOLERANCE", "OUT_OF_TOLERANCE", "NO_MATCH"],
                weights=[60, 20, 15, 5]
            )[0]
            ...
            if category == "MATCHED":
                ...
                inv_qty = round(o_qty * random.uniform(0.97, 1.03), 2)
                inv_price = round(o_price * random.uniform(0.97, 1.03), 4)
                inv_sku = sku
            elif category == "OUT_OF_TOLERANCE":
                inv_qty = round(o_qty * random.uniform(0.80, 1.20), 2)
                inv_price = round(o_price * random.uniform(0.80, 1.20), 4)
                inv_sku = sku
            else:  # NO_MATCH
...
def seed_reconciliations(conn, invoice_records, inv_line_records):
    ...
    for invoice_id, vendor_id, po_number in invoice_records:
        lines = lines_by_invoice.get(invoice_id, [])
        categories = [row[9] for row in lines]

        # Determine overall_status from line categories
        if all(c == "MATCHED" for c in categories):
            overall_status = "MATCHED"
            confidence = round(random.uniform(0.90, 1.00), 4)
        elif any(c in ("OUT_OF_TOLERANCE", "NO_MATCH") for c in categories):
        ...
        for row in lines:
            ...
            match_status = category  # already one of our status values
            qty_diff = round(inv_qty - ord_qty, 4)
            price_diff = round(inv_price - ord_price, 4)

            applied_rule = {
                "MATCHED": "EXACT_MATCH",
                "WITHIN_TOLERANCE": "TOLERANCE_RULE_5PCT",
                "OUT_OF_TOLERANCE": "NO_RULE_APPLIED",
                "NO_MATCH": "NO_RULE_APPLIED",
            }[category]
            ...
            # Exceptions only for OUT_OF_TOLERANCE and NO_MATCH
            if category in ("OUT_OF_TOLERANCE", "NO_MATCH"):
                if category == "NO_MATCH":
                    exc_type = "MISSING_LINE"
                    severity = "HIGH"
                    desc = (f"Product code mismatch: invoice has {inv_sku}, "
                            f"PO has {ord_sku}")
                    auto_action = "HOLD_FOR_REVIEW"
                else:
                    exc_type = random.choice(["PRICE_DISCREPANCY", "QTY_DISCREPANCY"])
                    severity = random.choice(["MEDIUM", "HIGH"])
                    if exc_type == "PRICE_DISCREPANCY":
                        pct = abs(price_diff / ord_price * 100) if ord_price else 0
                        desc = (f"Price diff {price_diff:+.2f} "
                                f"({pct:.1f}% variance)")
                    else:
                        pct = abs(qty_diff / ord_qty * 100) if ord_qty else 0
                        desc = (f"Qty diff {qty_diff:+.2f} "
                                f"({pct:.1f}% variance)")
                    auto_action = "FLAG_FOR_APPROVAL"
...
def seed_pipeline_logs(conn, invoice_records, recon_ids):
    ...
    for invoice_id, vendor_id, po_number in invoice_records:
        ...
        log_entries = [
            ...
            (
                "MATCHER", "INFO",
                f"[MATCHER] Starting matching for invoice_id={invoice_id} po_number={po_number}",
                base_ts + timedelta(milliseconds=random.randint(1500, 2200)),
            ),
            (
                "MATCHER", "INFO",
                f"[MATCHER] Done. recon_id={recon_id}",
                base_ts + timedelta(milliseconds=random.randint(2200, 4500)),
            ),
            (
                "EXCEPTION_HANDLER", "INFO",
                f"[EXCEPTION_HANDLER] Scanning recon_id={recon_id} for discrepancies",
                base_ts + timedelta(milliseconds=random.randint(4500, 5500)),
            ),
            (
                "EXCEPTION_HANDLER", "INFO",
                f"[EXCEPTION_HANDLER] Completed for invoice_id={invoice_id}",
